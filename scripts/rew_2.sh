...
export TMUX_TMPDIR="/data/user_data/sanidhyv/ray_temp_s/tmux_sessions"
mkdir -p "$TMUX_TMPDIR"
...
rm -r /tmp/ray_sanidhyv 2>/dev/null || true
...
export TMPDIR="/data/user_data/sanidhyv/tmp2"
export RAY_TMPDIR="/tmp/ray_sanidhyv"
...
CKPT_PATH="/data/user_data/sanidhyv/agentic-code-search-oss/ckpts_rew_4/code_search_${MODEL_ALIAS}_batched"
CACHE_DIR="/data/user_data/sanidhyv/tmp/embedding_cache2"

# Batched indexing configuration
BATCH_SIZE=15  # Number of repos per batch
REPOS_DIR="/data/user_data/sanidhyv/grep2"  # Where repos are cloned

# Training configuration
N_ROLLOUTS="${N_ROLLOUTS:-4}"
TRAIN_BATCH_SIZE=4
MAX_LENGTH=2048
export WANDB_PROJECT="code_search_4b_rew_4"
...
export RAY_object_spilling_config='{"type":"filesystem","params":{"directory_path":"/data/user_data/sanidhyv/ray_spill2"}}'
export HYDRA_FULL_ERROR=1
MAX_RESTART_ATTEMPTS=20
RESTART_DELAY=300
mkdir -p /data/user_data/sanidhyv/ray_spill2
# rm -r ckpts
# ============================================================================
...
for attempt in $(seq 1 $MAX_RESTART_ATTEMPTS); do
    ...
    +generator.reward=configs/rewards/anti_shortcut.yaml \
